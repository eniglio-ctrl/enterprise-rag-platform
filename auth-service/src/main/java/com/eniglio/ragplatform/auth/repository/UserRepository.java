...
public class UserRepository {
    ...
    public Optional<User> findByEmail(String email) {
        return jdbcTemplate.query(
                        "SELECT id, tenant_id, email, password_hash, role FROM users WHERE lower(email) = lower(?)",
                        USER_MAPPER, email)
                .stream()
                .findFirst();
    }
    ...
    public List<User> findByTenantId(String tenantId) {
        return jdbcTemplate.query(
                "SELECT id, tenant_id, email, password_hash, role FROM users WHERE tenant_id = ? ORDER BY email",
