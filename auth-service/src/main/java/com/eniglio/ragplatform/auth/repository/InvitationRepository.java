...
public class InvitationRepository {
    ...
    public Invitation create(String tenantId, String email, Role role, Instant expiresAt) {
        UUID id = UUID.randomUUID();
        String token = UUID.randomUUID().toString();
        jdbcTemplate.update(
                "INSERT INTO invitations (id, tenant_id, email, token, role, expires_at) VALUES (?, ?, ?, ?, ?, ?)",
                id, tenantId, email, token, role.name(), Timestamp.from(expiresAt));
        return new Invitation(id.toString(), tenantId, email, token, role, expiresAt, null, Instant.now());
    }

    public Optional<Invitation> findByToken(String token) {
        return jdbcTemplate.query(
                        "SELECT id, tenant_id, email, token, role, expires_at, redeemed_at, created_at "
    ...
    public Optional<Invitation> redeem(String token) {
        return jdbcTemplate.query(
                        "UPDATE invitations SET redeemed_at = now() "
                                ...
                                + "RETURNING id, tenant_id, email, token, role, expires_at, redeemed_at, created_at",
                        INVITATION_MAPPER, token)
                .stream()
                .findFirst();
    }
