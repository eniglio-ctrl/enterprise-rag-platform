...
public class GlobalExceptionHandler extends GlobalExceptionHandlerSupport {
    ...
    @ExceptionHandler(DepartmentRequestNotFoundException.class)
    public ResponseEntity<ErrorResponse> handleDepartmentRequestNotFound(DepartmentRequestNotFoundException ex,
            HttpServletRequest request) {
        return build(HttpStatus.NOT_FOUND, ex.getMessage(), request);
    }
