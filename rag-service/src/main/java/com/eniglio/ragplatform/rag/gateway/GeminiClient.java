...
public class GeminiClient {
    ...
    public GenerationResult generateContent(String question) {
    ...
    private static GenerationResult toResult(GeminiResponse response) {
        ...
        // docs/adr/0056-llm-cost-metering-for-fallback-calls.md: usageMetadata is a
        // real field the Generative Language API's generateContent response already
        // returns, just never read here before - missing entirely only if the API
        // response shape itself changes, which promptTokenCount/candidatesTokenCount
        // defaulting to 0 below handles the same "unknown, not free" way
        // CostMeteringService treats any provider's unreported usage.
        GeminiResponse.UsageMetadata usage = response.usageMetadata();
        int promptTokens = usage != null && usage.promptTokenCount() != null ? usage.promptTokenCount() : 0;
        int completionTokens = usage != null && usage.candidatesTokenCount() != null ? usage.candidatesTokenCount() : 0;
        return new GenerationResult(parts.get(0).text(), promptTokens, completionTokens);
    }
    ...
    private record GeminiResponse(List<Candidate> candidates, UsageMetadata usageMetadata) {
        ...
        /** Deliberately doesn't model {@code totalTokenCount} - promptTokens + completionTokens already sums it. */
