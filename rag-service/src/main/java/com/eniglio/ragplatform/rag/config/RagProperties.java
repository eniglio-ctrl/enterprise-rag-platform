...
public record RagProperties(int topK, double similarityThreshold, int rerankCandidatePoolSize,
    ...
    /**
     * docs/adr/0057-document-comparison.md. {@code maxDocuments} bounds how many
     * documents a single comparison request can process at once - each document is
     * already individually truncated by {@link DocumentInsights#maxChunks()}; this
     * bounds the other axis (how many documents), not a combined-content cap, same
     * "explicit, documented limit" philosophy as every other cap in this codebase.
     */
    public record DocumentComparison(int maxDocuments) {
    }
}
