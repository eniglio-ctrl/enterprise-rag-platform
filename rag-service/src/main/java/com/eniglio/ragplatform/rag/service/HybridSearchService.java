...
public class HybridSearchService {
     ...
     * -membership-and-approval.md: {@code departments} (the caller's own, from their
     * JWT claim, empty if they have none) is threaded through the exact same
     * {@link #filterVisible} call - a second, independent way in alongside
     * {@code userId}, not a replacement for it.
     */
    public List<Document> search(String question, String tenantId, String userId, List<String> departments,
            int topK) {
        ...
        SearchRequest vectorRequest = SearchRequest.builder()
                ...
                .build();
        List<Document> vectorResults = filterLatestVersion(
                filterVisible(vectorStoreGateway.search(vectorRequest), userId, departments));
        List<Document> textResults = filterLatestVersion(
                filterVisible(fullTextSearch(question, tenantId, poolSize), userId, departments));
     ...
     * docs/adr/0058-document-versioning.md: two documentIds can share the same
     * {@code source} filename (nothing prevents re-uploading a file without using
     * {@code supersedes}), so this also filters to the latest version only, same as
     * {@link #search} - a tool-invoked lookup is "a normal question" in the same sense.
     */
    public List<Document> findBySource(String source, String tenantId, String userId, List<String> departments) {
        List<Document> chunks = jdbcTemplate.query(LOOKUP_BY_SOURCE_SQL,
                (rs, rowNum) -> Document.builder()
                        ...
                        .build(),
                source, tenantId);
        return filterLatestVersion(filterVisible(chunks, userId, departments));
    }
     ...
     * docs/adr/0058-document-versioning.md: deliberately NOT filtered to the latest
     * version - an exact {@code documentId} lookup already IS "ask against a specific
     * version," the exact mechanism that must keep working after versioning exists.
     */
    public List<Document> findByDocumentId(String documentId, String tenantId, String userId,
        ...
        List<Document> chunks = jdbcTemplate.query(LOOKUP_BY_DOCUMENT_ID_SQL,
                (rs, rowNum) -> Document.builder()
                        ...
                        .build(),
                documentId, tenantId);
        return filterVisible(chunks, userId, departments);
    }

    private List<Document> filterVisible(List<Document> documents, String userId, List<String> departments) {
        return documents.stream()
                .filter(document -> DocumentVisibility.isVisibleTo(document.getMetadata(), userId, departments))
                .toList();
    }

    /**
     * docs/adr/0058-document-versioning.md: same shape/position as {@link
     * #filterVisible} and for the same reason - Spring AI's filter DSL can't express
     * "missing key = default true", so the vector leg's candidates (which only carry
     * raw metadata JSON, not the generated {@code is_latest_version} column) need this
     * applied in Java, same as the full-text leg, to keep both legs consistent.
     * Package-private so it can be unit tested directly, same reason as {@link
     * #fuseWithRrf}.
     */
    List<Document> filterLatestVersion(List<Document> documents) {
        return documents.stream()
                .filter(document -> DocumentVersion.isLatestVersion(document.getMetadata()))
                .toList();
    }
