...
class HybridSearchServiceTest {
    ...
    @Test
    void filterLatestVersionKeepsDocumentsWithNoVersioningMetadata() {
        Document noVersionMetadata = Document.builder().id("d1").text("t").metadata(Map.of()).build();

        List<Document> filtered = service.filterLatestVersion(List.of(noVersionMetadata));

        assertThat(filtered).containsExactly(noVersionMetadata);
    }

    @Test
    void filterLatestVersionDropsSupersededDocuments() {
        Document superseded = Document.builder().id("d1").text("t")
                .metadata(Map.of("isLatestVersion", false)).build();
        Document latest = Document.builder().id("d2").text("t")
                .metadata(Map.of("isLatestVersion", true)).build();

        List<Document> filtered = service.filterLatestVersion(List.of(superseded, latest));
