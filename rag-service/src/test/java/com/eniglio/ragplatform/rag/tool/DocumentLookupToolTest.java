...
class DocumentLookupToolTest {
    ...
    void concatenatesEveryChunkOfTheFoundDocumentInOrder() {
        ...
        given(hybridSearchService.findBySource("saga-pattern.txt", "acme", null)).willReturn(List.of(chunk0, chunk1));
    ...
    void tenantIdComesFromToolContextNeverFromTheModelSuppliedParameter() {
        given(hybridSearchService.findBySource(eq("saga-pattern.txt"), eq("acme"), eq(null))).willReturn(List.of());
        ...
        verify(hybridSearchService).findBySource("saga-pattern.txt", "acme", null);
    }
    ...
    void userIdComesFromToolContextNeverFromTheModelSuppliedParameter() {
        ...
        given(hybridSearchService.findBySource(eq("saga-pattern.txt"), eq("acme"), eq("user-1"))).willReturn(List.of());

        new DocumentLookupTool(hybridSearchService)
                .lookupDocumentBySource("saga-pattern.txt", new ToolContext(Map.of("tenantId", "acme", "userId", "user-1")));

        verify(hybridSearchService).findBySource("saga-pattern.txt", "acme", "user-1");
    }

    @Test
    void returnsAClearMessageWhenNoDocumentMatchesTheGivenSource() {
        given(hybridSearchService.findBySource("does-not-exist.txt", "acme", null)).willReturn(List.of());
