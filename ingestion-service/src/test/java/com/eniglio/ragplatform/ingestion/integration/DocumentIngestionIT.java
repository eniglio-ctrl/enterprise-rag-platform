...
class DocumentIngestionIT {
    ...
    void uploadingARealPdfIngestsItIntoTheVectorStore() throws Exception {
        stubEmbeddingModel();
        // docs/adr/0055-ocr-fallback-for-scanned-pdfs.md: minimalPdfBytes() is a single
        // blank (textless) page, which PagePdfDocumentReader drops entirely - before
        // this ADR that meant a silent 201 with pageCount/chunkCount 0. Now it goes
        // through the vision fallback, hence this stub, matching the PNG test above.
        given(chatModel.call(any(Prompt.class))).willReturn(new ChatResponse(
                List.of(new Generation(new AssistantMessage("A blank scanned page.")))));
    ...
    @Test
    void uploadingAPdfWithRealTextNeverCallsTheVisionModel() throws Exception {
        // docs/adr/0055-ocr-fallback-for-scanned-pdfs.md regression coverage: a real
        // bug slipped through unit tests that hand-built the extracted Document's
        // page_number instead of running the actual PagePdfDocumentReader, which
        // turned out to number pages 1-indexed. That mismatch made
        // ScannedPageVisionFallbackService treat a page that already had real text
        // as "missing" and needlessly render+describe it via the vision model,
        // duplicating the page. This test uses a genuinely real single-page PDF
        // (unlike minimalPdfBytes() above, which is deliberately blank) and asserts
        // the vision model is never invoked and the page isn't duplicated.
        stubEmbeddingModel();

        MockMultipartFile file = new MockMultipartFile(
                "file", "real-text.pdf", "application/pdf", pdfBytesWithRealText());

        mockMvc.perform(multipart("/api/v1/documents").file(file)
                        .with(jwt().jwt(token -> token.subject("user-1").claim("tenantId", "acme"))))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.source").value("real-text.pdf"))
                .andExpect(jsonPath("$.pageCount").value(1))
                .andExpect(jsonPath("$.chunkCount").value(1));
        ...
        mockMvc.perform(patch("/api/v1/documents/{documentId}/sharing", documentId)
                        ...
                        .content(objectMapper.writeValueAsString(
                                new com.eniglio.ragplatform.ingestion.dto.UpdateSharingRequest(
                                        "RESTRICTED", List.of("user-shared"), List.of()))))
    ...
    void aNonOwnerCannotChangeAnotherUsersDocumentSharing() throws Exception {
        ...
        mockMvc.perform(patch("/api/v1/documents/{documentId}/sharing", documentId)
                        ...
                        .contentType("application/json")
                        .content(objectMapper.writeValueAsString(
                                new com.eniglio.ragplatform.ingestion.dto.UpdateSharingRequest("RESTRICTED", List.of(), List.of()))))
    ...
    void aVisibilityValueOtherThanTenantOrRestrictedReturns400() throws Exception {
        ...
        mockMvc.perform(patch("/api/v1/documents/{documentId}/sharing", documentId)
                        ...
                        .contentType("application/json")
                        .content(objectMapper.writeValueAsString(
                                new com.eniglio.ragplatform.ingestion.dto.UpdateSharingRequest("PUBLIC", List.of(), List.of()))))
    ...
    @Test
    void uploadingANewVersionSupersedesTheOldOneAndLinksThemByGroup() throws Exception {
        stubEmbeddingModel();
        String v1DocumentId = uploadMarkdown("aula12.md", "user-owner", "acme");

        MockMultipartFile v2File = new MockMultipartFile("file", "aula12-v2.md", "text/markdown",
                "# Aula 12 (revisada)\n\nO padrão SAGA agora usa apenas orchestration.".getBytes(StandardCharsets.UTF_8));

        MvcResult result = mockMvc.perform(multipart("/api/v1/documents")
                        .file(v2File)
                        .param("supersedes", v1DocumentId)
                        .with(jwt().jwt(token -> token.subject("user-owner").claim("tenantId", "acme"))))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.documentGroupId").value(v1DocumentId))
                .andExpect(jsonPath("$.version").value(2))
                .andReturn();
        String v2DocumentId = objectMapper.readTree(result.getResponse().getContentAsString())
                .get("documentId").asText();

        Boolean v1StillLatest = jdbcTemplate.queryForObject(
                "SELECT (metadata->>'isLatestVersion')::boolean IS NOT FALSE FROM vector_store "
                        + "WHERE metadata->>'documentId' = ? LIMIT 1",
                Boolean.class, v1DocumentId);
        assertThat(v1StillLatest).isFalse();

        Boolean v2IsLatest = jdbcTemplate.queryForObject(
                "SELECT (metadata->>'isLatestVersion')::boolean IS NOT FALSE FROM vector_store "
                        + "WHERE metadata->>'documentId' = ? LIMIT 1",
                Boolean.class, v2DocumentId);
        assertThat(v2IsLatest).isTrue();
    }

    @Test
    void anAlreadySupersededVersionCannotBeSupersededAgain() throws Exception {
        stubEmbeddingModel();
        String v1DocumentId = uploadMarkdown("aula12.md", "user-owner", "acme");

        MockMultipartFile v2File = new MockMultipartFile("file", "aula12-v2.md", "text/markdown",
                "# Aula 12 (revisada)".getBytes(StandardCharsets.UTF_8));
        mockMvc.perform(multipart("/api/v1/documents").file(v2File)
                        .param("supersedes", v1DocumentId)
                        .with(jwt().jwt(token -> token.subject("user-owner").claim("tenantId", "acme"))))
                .andExpect(status().isCreated());

        MockMultipartFile v3File = new MockMultipartFile("file", "aula12-v3.md", "text/markdown",
                "# Aula 12 (tentativa de superar a v1 de novo)".getBytes(StandardCharsets.UTF_8));
        mockMvc.perform(multipart("/api/v1/documents").file(v3File)
                        .param("supersedes", v1DocumentId)
                        .with(jwt().jwt(token -> token.subject("user-owner").claim("tenantId", "acme"))))
    ...
    @Test
    void aNonOwnerCannotUploadANewVersionOfSomeoneElsesDocument() throws Exception {
        stubEmbeddingModel();
        String v1DocumentId = uploadMarkdown("aula12.md", "user-owner", "acme");
    ...
    @Test
    void supersedingAnUnknownDocumentReturns404() throws Exception {
        MockMultipartFile v2File = new MockMultipartFile("file", "aula12-v2.md", "text/markdown",
                "# Aula 12".getBytes(StandardCharsets.UTF_8));
        mockMvc.perform(multipart("/api/v1/documents").file(v2File)
                        .param("supersedes", "does-not-exist")
                        .with(jwt().jwt(token -> token.subject("user-owner").claim("tenantId", "acme"))))
                .andExpect(status().isNotFound());
    }
    ...
    private static byte[] pdfBytesWithRealText() throws IOException {
        try (PDDocument document = new PDDocument()) {
            ...
            try (PDPageContentStream contentStream = new PDPageContentStream(document, page)) {
                ...
                contentStream.showText("A real, non-scanned PDF page with an embedded text layer.");
