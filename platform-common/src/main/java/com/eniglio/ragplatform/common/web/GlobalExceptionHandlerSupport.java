...
public abstract class GlobalExceptionHandlerSupport {
    ...
    /**
     * A real bug found live: a read timeout while Ollama is still streaming its
     * response body (e.g. a slow local generation for a large prompt, like
     * comparing two large real documents) surfaces as a bare {@link
     * RestClientException} wrapping a {@code SocketTimeoutException} - a different
     * failure point from {@link ResourceAccessException} above (which covers the
     * request never connecting at all). Without this handler it fell through to the
     * generic catch-all below, returning an unhelpful, English-only "An unexpected
     * error occurred" instead of the same friendly, actionable message every other
     * Ollama-unavailability case already gets. {@link ResourceAccessException}
     * extends this class, so Spring still resolves to its own, more specific
     * handler above for that case - this one only catches what that one doesn't.
     */
    @ExceptionHandler(RestClientException.class)
    public ResponseEntity<ErrorResponse> handleModelProviderCallFailure(RestClientException ex, HttpServletRequest request) {
        log.error("Unexpected failure calling a model provider", ex);
        return build(HttpStatus.SERVICE_UNAVAILABLE,
                "O serviço de IA está temporariamente indisponível. Tente novamente em instantes.", request);
    }
