...
Every real run now appends one row (date, git commit, similarity, faithfulness,
context-relevance, latency) to the git-tracked
`rag-service/src/test/resources/benchmark/history.csv` — added in Multi-LLM Phase
16 (ADR 0053) specifically so a later run can be diffed against an earlier one
instead of only ever comparing against whatever number happened to be written
here. **Latest real run (2026-08-08, after fixing a real `unaccent_simple`
regression both benchmark classes had silently had since ADR 0042): average
similarity 0.588** across 10 questions (minimum bar: 0.60 — **currently not
met**), 8/10 answers faithful, average context-relevance 0.20 (explained, not a
bug — the benchmark's 10 QA pairs share one tenant, so a 5-chunk retrieval sees
the whole 10-document pool with only one truly relevant document per question;
see ADR 0034). The similarity miss is a genuine, open finding — flagged in ADR
0053, not investigated further there — not yet root-caused against the original
0.651 this file used to report. Several answers came back in Portuguese for
English questions — a genuine, CPU-bound `llama3.1` quirk on this hardware, not
a retrieval defect — which cross-lingual cosine similarity penalizes even when
the answer is factually correct; see the class's own Javadoc for the full
...
A separate, opt-in Cypress suite drives a real browser against the real
`docker compose` stack — `web-ui` and all four Java services, no mocking —
covering the highest-value flows: register/login, upload + URL import +
asking a question, per-document summarize/FAQ (plus the real SSRF-guard
error path), and admin document sharing. Added in Multi-LLM Phase 17
(ADR 0054), which also found and fixed two real, pre-existing bugs
(`platform-common`'s CORS config never allowed `PATCH`, silently blocking
the document-sharing and role-promotion endpoints in any real browser; and
`web-ui`'s `setStatus()` silently destroyed an element's extra CSS classes
on its first status update) — both invisible until a real browser actually
exercised those exact clicks.

```bash
docker compose up -d          # the full local stack must already be running
cd web-ui
npm install
npx cypress run               # headless, or `npx cypress open` for the interactive runner
```

Needs a real, reachable local Ollama with `llama3.1`/`nomic-embed-text`
already pulled (same requirement as the RAG quality benchmark above), so
like that benchmark, it's not wired into the hosted GitHub Actions CI yet —
see ADR 0054 for the reasoning and the exact follow-up that would change
that.
