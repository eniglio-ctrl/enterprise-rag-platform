...
any of them existed. Fifty-one items are tracked across all six places (up
...
49. ⬜ **New — Product Differentiation Phase 10: Audio Overview** (a
    ...
    [docs/PRODUCT-DIFFERENTIATION-ROADMAP.md](PRODUCT-DIFFERENTIATION-ROADMAP.md)
    Phase 10.
50. ⬜ **New — Product Differentiation Phase 11: document deletion**
    (added 2026-08-11, direct follow-up request). No `DELETE` endpoint
    exists anywhere in `ingestion-service` today — a document can be
    superseded (ADR 0058) but never actually removed. Self-contained, no
    blocker, same "no infra missing" shape as the phases that already
    landed this session. See
    [docs/PRODUCT-DIFFERENTIATION-ROADMAP.md](PRODUCT-DIFFERENTIATION-ROADMAP.md)
    Phase 11.
51. ⬜ **New — Product Differentiation Phase 12: department-based document
    sharing** (added 2026-08-11, direct follow-up request). Today's ABAC
    model (ADR 0046) only ever shares a document tenant-wide or with an
    explicit, manually-picked list of individual users — no group/
    department tier in between. Touches three services (`auth-service`
    for a new `department` field + JWT claim, `platform-common`'s
    `DocumentVisibility`, `ingestion-service`/`rag-service` for the write/
    read sides) — bigger than most recently-landed phases, but still no
    new infra or external account needed. Also the real primitive Phase 3
    (specialized domain agents) already assumes exists. See
    [docs/PRODUCT-DIFFERENTIATION-ROADMAP.md](PRODUCT-DIFFERENTIATION-ROADMAP.md)
    Phase 12.
