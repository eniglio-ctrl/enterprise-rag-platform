...
| 2 | Import a whole local folder (client-side, multi-file) | ✅ Done (2026-08-11) | — |
...
**Done (2026-08-11).** `web-ui` only, no backend change, exactly as
planned. A new `#folder-input` (`<input type="file" webkitdirectory
multiple hidden>`) next to the existing dropzone gives a flat `FileList`
of every file inside the chosen folder (recursively), each a normal
`File` object. The single POST call the dropzone already made
(`POST ${INGESTION_BASE}/api/v1/documents`) was extracted into a shared
`uploadFile(file)` in `web-ui/app.js`, reused by both the unchanged
single-file form and a new sequential loop over the folder's files: a
file whose extension isn't in the same accepted list as the dropzone's
own `accept` attribute is skipped without calling the backend at all; a
file the backend itself rejects (415/422) is skipped with the server's
own message as the reason. Both render via a new
`addSkippedHistoryEntry` in the same `#upload-history` list a successful
import renders in (visually dimmer, via a new `.history-skipped` style),
so a folder with a mix of file types reads as one combined feed —
imported and skipped side by side, each with its own visible outcome.
...
unchanged. Verified: a new Cypress test
(`web-ui/cypress/e2e/documents.cy.js`) selects a real supported file plus
a real unsupported one via `#folder-input` against the live stack and
asserts exactly one imported row and one skipped row (with the reason
text) appear; the existing single-file upload test in the same spec
still passes unchanged, proving the extraction didn't alter that path's
behavior. Full Cypress suite (11 tests across 4 specs) green.
...
MCP client (a desktop AI assistant, an IDE, or any other MCP-speaking tool) can connect
