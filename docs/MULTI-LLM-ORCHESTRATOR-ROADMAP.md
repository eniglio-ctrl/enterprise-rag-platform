...
| 17 | Cypress E2E suite for `web-ui` | ✅ Done — [ADR 0054](adr/0054-cypress-e2e-suite-for-web-ui.md) | — |
...
ChatGPT/Gemini/Anthropic as parallel providers behind a shared `AIProvider`
...
- **None of this infrastructure exists today.** No Redis, no Kafka, no MCP, no
  LangFuse, no OpenTelemetry/distributed tracing, no Anthropic, no
...
## Phase 17 — Cypress E2E suite for `web-ui` ✅

**Done (2026-08-08).** See [ADR 0054](adr/0054-cypress-e2e-suite-for-web-ui.md)
for the full account. Added directly after the user asked whether Helm and
Cypress belonged in this project — Helm was rejected (Kustomize already
covers the same need, not even using overlays yet), Cypress had a real
gap to fill: `web-ui` had zero automated E2E coverage before this.

Four specs (`web-ui/cypress/e2e/`) against the real local stack, no
mocking: `auth.cy.js` (register/login/logout, wrong-password rejection),
...
`admin.cy.js` (admin sections, document sharing).

**Two real, pre-existing bugs found and fixed** by actually clicking
through the app instead of reasoning about it — invisible to every
existing backend test and to manual testing so far:
- `platform-common`'s shared `CorsConfig` never allowed the `PATCH` method,
  silently blocking both `ingestion-service`'s document-sharing endpoint
  and `auth-service`'s role-promotion endpoint in any real browser.
- `web-ui/app.js`'s `setStatus()` overwrote an element's whole
  `className`, silently destroying any class beyond `"status"` the first
  time a status message appeared - invisible to users, but broke any code
  trying to re-select that element by its specific class afterward.

**Done when**: `npx cypress run` (all four specs) passes against the real
docker-compose stack. Confirmed twice, clean, 10/10, after both bugs above
were fixed and rebuilt. Not wired into the hosted GitHub Actions CI yet -
same "needs a real local Ollama, which a GitHub-hosted runner doesn't
have" reasoning already applied to `RagQualityBenchmark` (Phase 16); a
self-hosted runner with Ollama pre-pulled would be the natural follow-up
if this project ever wants that gate automated.
