...
## Amendment (2026-08-08): FAQ generation hardened after a real failure

Real usage (a denser, formal PDF, not this project's usual Markdown fixtures) hit
exactly the failure mode this ADR anticipated: `FaqGenerationException` on the first
try. Two gaps found from that report, both closed:

- **The raw response that failed to parse was never logged anywhere** — the only
  trace was the generic error message reaching `web-ui`. Fixed: a failed parse now
  logs the raw model text at `WARN` before throwing, so a future failure is
  diagnosable from `docker logs` instead of only reproducible by re-clicking the
  button and hoping.
- **No retry, no few-shot example, and an overly literal parser.** `FAQ_SYSTEM_TEMPLATE`
  now includes a concrete (unrelated-topic) example of the exact `P:`/`R:` syntax —
  known to meaningfully improve local-model format compliance. `doGenerateFaq` now
  retries once, with a blunter `FAQ_RETRY_REMINDER` prepended, before giving up —
  one extra local-model call, only on the failure path. `FAQ_BLOCK`'s regex was
  loosened to also accept `Pergunta:`/`Resposta:` (not just `P:`/`R:`) and an
  optional leading bullet/number, since a model asked for the literal short form
  still sometimes spells the labels out.

Not changed: still fails loudly (`FaqGenerationException`, 500) if both the original
attempt and the retry produce nothing parseable — an empty FAQ list still shouldn't
look like a quiet success. Verified via new unit tests
(`parsesFaqPairsUsingThePerguntaRespostaLabelVariant`,
`faqGenerationRetriesOnceAndRecoversFromAMalformedFirstAttempt`,
`RagQueryServiceTest.java`) and the full existing suite staying green; not verified
against the exact document that originally failed, since its raw model output was
never captured (the logging gap above is what would have made that possible).
